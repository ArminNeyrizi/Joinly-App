...
import type { EnrollmentPageData, EnrollmentStatus } from "../types";
...
const statusLabel: Record<EnrollmentStatus, string> = {
  PENDING: "در سبد (ثبت نشده)",
  HOLD: "در انتظار تایید ادمین",
  APPROVED: "تایید شده",
  REJECTED: "رد شده",
  DROPPED: "حذف شده",
};
...
// دانش‌آموز فقط تا وقتی که ادمین تایید نکرده می‌تواند درس را حذف کند
const DROPPABLE_STATUSES: EnrollmentStatus[] = ["PENDING", "HOLD", "REJECTED"];
...
export function SelectedCourses({
  ...
  function handleConfirm() {
    startTransition(async () => {
      ...
      if (result.success) {
        onUpdate(result.data);
        onSuccess("انتخاب واحد ثبت شد و برای بررسی نزد ادمین ارسال شد");
      } else {
        onError("خطایی در ثبت نهایی رخ داد");
      }
    });
  }
    ...
    <div className="space-y-4">
          ...
          >
            <CardHeader className="flex-row items-start justify-between gap-4">
              ...
              <div className="flex items-center gap-2">
                ...
                {DROPPABLE_STATUSES.includes(status) &&
            ...
            <CardContent className="space-y-2 text-sm text-muted-foreground">
              ...
              {status === "REJECTED" && item.enrollment.rejectReason && (
                <p className="flex items-start gap-1.5 text-xs text-red-400">
                  <AlertTriangle className="mt-0.5 size-3.5 shrink-0" />
                  دلیل رد: {item.enrollment.rejectReason}
                </p>
              )}
            </CardContent>
          </Card>
      ...
      <div className="sticky bottom-4 rounded-xl border border-border/50 bg-card/90 p-4 backdrop-blur-xl">
        ...
        {pendingCount > 0 && data.summary.isEnrollmentOpen && (
          ...
          >
            {isPending ? "در حال ثبت..." : "ثبت نهایی و ارسال برای تایید ادمین"}
          </Button>
        )}
      </div>
    </div>
